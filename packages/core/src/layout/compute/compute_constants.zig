...
pub const AvailableSpace = union(enum) {
    ...
    pub fn maybeAddIfDefinite(self: AvailableSpace, other: anytype) AvailableSpace {
        switch (self) {
            .definite => |v| return .{ .definite = Maybe.add(v, other) },
            else => return self,
        }
    }
    ...
    pub fn maybeSubtractIfDefinite(self: AvailableSpace, other: anytype) AvailableSpace {
        switch (self) {
            .definite => |v| return .{ .definite = Maybe.sub(v, other) },
            else => return self,
        }
    }
    ...
    pub fn maybeClamp(self: AvailableSpace, min: anytype, max: anytype) AvailableSpace {
        switch (self) {
            .definite => |v| return .{ .definite = Maybe.clamp(v, min, max) },
            else => return self,
        }
    }
    ...
    pub fn maybeMax(self: AvailableSpace, other: anytype) AvailableSpace {
        switch (self) {
            .definite => |v| return .{ .definite = Maybe.max(v, other) },
            else => return self,
        }
    }
    pub fn maybeMin(self: AvailableSpace, other: anytype) AvailableSpace {
        switch (self) {
            .definite => |v| return .{ .definite = Maybe.min(v, other) },
            else => return self,
        }
    }
    ...
    pub fn fromPoint(point: anytype) Point(AvailableSpace) {
        if (@TypeOf(point) == Point(f32)) {
            return .{
                .x = .{ .definite = point.x },
                .y = .{ .definite = point.y },
            };
    ...
    pub const MAX_CONTENT: Point(AvailableSpace) = .{ .x = .max_content, .y = .max_content };
    pub const MIN_CONTENT: Point(AvailableSpace) = .{ .x = .min_content, .y = .min_content };
};
...
pub const AbsoluteAxis = enum {
    ...
    pub fn fromFlexDirection(direction: styles.flex_direction.FlexDirection) AbsoluteAxis {
        switch (direction) {
