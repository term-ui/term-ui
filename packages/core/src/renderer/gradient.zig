...
test "radial-tests" {
    // const size: Point(f32) = .{ .x = 100, .y = 50 };
    // const center: Point(f32) = .{ .x = 50, .y = 25 };
    // const radii = radial.ellipseRadiiClosestCorner(center, size);
    // std.debug.print("size: {any} center: {any} radii: {any}\n", .{ size, center, radii });
}
test "samplegradient" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    const size: Point(u32) = .{ .x = 100, .y = 50 };
    defer arena.deinit();
    const allocator = arena.allocator();

    var canvas = try Canvas.init(
        allocator,
        size, // Larger canvas size
        Color.tw.white,
        Color.tw.white,
    );
    defer canvas.deinit();

    // const linear_gradient = try styles.linear_gradient.parse(
    //     allocator,
    //     "linear-gradient(90deg, red, transparent, blue)",
    //     0,
    // );
    // const stops = linear_gradient.value.color_stops;
    // const premultiplied_gradient_line = try renderGradientLine(allocator, stops, 100, true);
    // const unpremultiplied_gradient_line = try renderGradientLine(allocator, stops, 100, false);
    // // const result = sample(stops, t);
    // for (premultiplied_gradient_line.line, unpremultiplied_gradient_line.line, 0..) |color, unpremultiplied_color, i| {
    //     std.debug.print("color: {s} {s}\n", .{ color, unpremultiplied_color });

    //     // const color = gradient_line.line[i];
    //     try canvas.drawRectBg(
    //         .{
    //             .pos = .{ .x = @intCast(i), .y = 3 },
    //             .size = .{ .x = 1, .y = 1 },
    //         },
    ...
    //         .size = .{ .x = 100, .y = 10 },
    //     },
    //     .{ .linear_gradient = linear_gradient.value },
    // );

    // const stops = "pink,cyan 10%, cyan 100%, red";
    // const linear_gradient = try styles.linear_gradient.parse(
    //     allocator,
    //     "linear-gradient(0, " ++ stops ++ ")",
    //     0,
    // );
    // const linear_gradient_line = try renderGradientLine(allocator, linear_gradient.value.color_stops, 25, true);
    // try styles.color_stop.dumpColorStops(linear_gradient.value.color_stops, std.io.getStdErr().writer());
    // for (linear_gradient_line.line, 0..) |color, i| {
    //     try canvas.drawRectBg(
    //         .{
    //             .pos = .{ .x = @intCast(i), .y = 0 },
    //             .size = .{ .x = 1, .y = 1 },
    ...
    fmt.debug.print("radial_gradient: {any}\n", .{radial_gradient.value});
    // const gradient = try RadialGradientSampler.init(allocator, .{ .x = 100, .y = 50 }, radial_gradient.value, true);
    ...
    //         .size = size,
    //     },
    //     .{ .radial_gradient = radial_gradient.value },
    // );
    // // fmt.debug.print("computed_size: {any}\n", .{gradient});
    // // const vec = @Vector(4, f64){ 1, 2, 3, 4 };
    // try canvas.render(std.io.getStdErr().writer().any());
    // std.debug.print("{} rad size {}\n", .{ size, gradient.radial_size });
    // std.debug.print("vec: {any}\n", .{@reduce(.Mul, vec)});

    // const gradient = try ComputedGradient.init(allocator, .{ .x = 4, .y = 2 }, 90, stops, true);
    // std.debug.print("gradient: {d:}\n", .{gradient.gradient_line_length});
    // const color = gradient.at(.{ .x = 4, .y = 0 });
    // std.debug.print("color: {s}\n", .{color});
    // std.debug.print("gradient_line_length: {d:}\n", .{getGradientLineLength(100, 20, 0)});
}
