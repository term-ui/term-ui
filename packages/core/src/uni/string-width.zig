...
pub fn visibleCodepointWidth(code_point: u32, ambiguousAsWide: bool) u3 {
    return codepoint.visibleWidth32(code_point, ambiguousAsWide);
}

pub fn visibleCodepointWidthType(comptime T: type, code_point: T, ambiguousAsWide: bool) u3 {
    return codepoint.visibleWidth32(@as(u32, @intCast(code_point)), ambiguousAsWide);
}
...
pub const visible = struct {
    ...
    fn indexOfCharUsize(input: []const u8, c: u8) ?usize {
        return std.mem.indexOfScalar(u8, input, c);
    }
    fn indexOfChar16Usize(input: []const u16, c: u16) ?usize {
        return std.mem.indexOfScalar(u16, input, c);
    }
    ...
    fn visibleLatin1WidthExcludeANSIColors(input_: anytype) usize {
        ...
        const indexFn = if (comptime ElementType == u8) indexOfCharUsize else indexOfChar16Usize;
    ...
    fn firstNonASCII(input: []const u8) ?usize {
        for (input, 0..) |byte, i| {
            if (byte >= 128) {
                return i;
            }
    ...
    fn visibleUTF8WidthFn(input: []const u8, comptime asciiFn: anytype) usize {
        ...
        while (firstNonASCII(bytes)) |i| {
            ...
            const skip = wtf8ByteSequenceLengthWithInvalid(byte);
